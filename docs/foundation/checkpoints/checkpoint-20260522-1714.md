# Frontend recovery checkpoint: frontend-checkpoint-20260522-1714

**Timestamp:** 2026-05-22T22:14:28.851Z  
**Tag:** `frontend-checkpoint-20260522-1714`  
**Commit:** `8c63c91e9122140e61278f38976b6699993d125b`  
**Branch:** `main`  
**Subject:** feat: email OTP auth, gift claim flow, /join signup, library gifted label, crons

## Checkpoint note

Manual frontend recovery checkpoint

## Deployment URLs

| Role | URL |
|------|-----|
| Production | https://artist-platform-silk.vercel.app |
| Legacy | https://2mrrw-official.vercel.app |
| Vercel project | artist-platform |

## Foundation anchor (canonical)

| Field | Value |
|-------|-------|
| Anchor commit | `f78d6ecd368ef82ca5929b3ab4269f31804ed131` |
| Stable branch | `frontend-stable-foundation` |
| Documented at | 2026-05-19 |

## Dependency state

```json
{
  "packageJson": {
    "dependencies": {
      "@aws-sdk/client-s3": "^3.1051.0",
      "@aws-sdk/s3-request-presigner": "^3.1051.0",
      "@stripe/react-stripe-js": "6.2.0",
      "@stripe/stripe-js": "9.2.0",
      "@supabase/ssr": "0.10.3",
      "@supabase/supabase-js": "2.105.4",
      "@types/howler": "^2.2.13",
      "@types/three": "^0.184.1",
      "framer-motion": "12.38.0",
      "howler": "^2.2.4",
      "next": "16.2.4",
      "qrcode.react": "4.2.0",
      "react": "19.2.4",
      "react-dom": "19.2.4",
      "stripe": "22.0.2",
      "three": "^0.184.0",
      "zustand": "^5.0.13"
    },
    "devDependencies": {
      "@tailwindcss/postcss": "4.2.2",
      "eslint": "9.39.4",
      "eslint-config-next": "16.2.4",
      "tailwindcss": "4.2.2",
      "vercel": "54.1.0"
    }
  },
  "anchorDependencies": {
    "next": "16.2.4",
    "react": "19.2.4",
    "react-dom": "19.2.4",
    "framer-motion": "12.38.0",
    "@supabase/ssr": "0.10.3",
    "@supabase/supabase-js": "2.105.4",
    "@stripe/react-stripe-js": "6.2.0",
    "@stripe/stripe-js": "9.2.0",
    "qrcode.react": "4.2.0",
    "stripe": "22.0.2",
    "@tailwindcss/postcss": "4.2.2",
    "tailwindcss": "4.2.2",
    "eslint": "9.39.4",
    "eslint-config-next": "16.2.4",
    "vercel": "54.1.0"
  },
  "lockfile": "package-lock.json"
}
```

## Package-lock verification

| Check | Result |
|-------|--------|
| Status | PASS |
| Detail | npm ci --dry-run passed |
| SHA-256 | `0a355b19029fe0c95f04fc150d48f2d6b7f085d0f7eb473bfbd3dbe78bc94899` |
| Size (bytes) | 411645 |

## Recovery instructions

```bash
git fetch --tags origin
git checkout frontend-checkpoint-20260522-1714
npm ci
npm run verify:foundation
npm run check:frontend-guardrails
```

Full foundation restore (sacred anchor, not this checkpoint):

```bash
npm run recover:foundation
```

## Rollback instructions

Return to stable foundation branch (does not delete checkpoints):

```bash
git fetch origin
git checkout frontend-stable-foundation
git pull --ff-only origin frontend-stable-foundation 2>/dev/null || true
npm ci
npm run verify:foundation
```

Sacred foundation tag (when promoted):

```bash
git fetch --tags origin
git checkout foundation-stable-v1
npm ci
npm run verify:foundation
```

## Push tag

```bash
git push origin frontend-checkpoint-20260522-1714
```

See [FRONTEND_FOUNDATION_TAG_STRATEGY.md](../FRONTEND_FOUNDATION_TAG_STRATEGY.md) and control repo `MILESTONE_RECOVERY_RECALL.md`.
