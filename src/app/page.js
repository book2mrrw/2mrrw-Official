...
export default function Page() {
  ...
  // §14 FLOW STATE — conversion active when user hovers radio Add to Cart
  ...
  // Now Playing sync — §13 FIX #4: src set here; play only if already playing
  useEffect(() => {
  ...
  // ── §13 FIX #3 — RADIO: auto-scroll FULLY REMOVED ─────────────────────
  // goRadio is a clean setter only — no intervals, no animation guards
  ...
  // §13 FIX #1 — hoverIn/hoverOut target ONLY the <img> element (used in Grid)
  ...
  // §13 FIX #4 — openSingleModal: pause nowPlayingRef BEFORE opening modal
  // prevents the modal <audio> and the background ref from playing simultaneously
  ...
  const switchTab  = (tabId) => { setTabKey((p)=>p+1); setActiveTab(tabId); };
  ...
  const FlowState = () => (
    <div style={{
      ...
      boxShadow: activeFlowMode==="conversion" ? `0 0 40px ${currentSlide.tagColor}20`
               : activeFlowMode==="nowplaying" ? `0 0 50px ${currentSlide.tagColor}12,inset 0 0 40px ${currentSlide.tagColor}06`
               : "none",
      ...
      <div style={{position:"absolute",inset:0,opacity:activeFlowMode==="idle"?1:0,pointerEvents:activeFlowMode==="idle"?"auto":"none",transition:"opacity 0.6s ease",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,gap:18,textAlign:"center"}}>
        ...
        <div style={{position:"relative",zIndex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:20}}>
          ...
          <div style={{display:"flex",gap:6}}>
            {[0,1,2].map((i)=>(
              <div key={i} style={{width:5,height:5,borderRadius:"50%",background:"rgba(0,255,255,0.18)",animation:`flowIdleDot 2.4s ease-in-out ${i*0.5}s infinite`}}/>
            ))}
          </div>
      ...
      <div style={{position:"absolute",inset:0,opacity:activeFlowMode==="nowplaying"?1:0,pointerEvents:activeFlowMode==="nowplaying"?"auto":"none",transition:"opacity 0.6s ease"}}>
        ...
        <div style={{position:"absolute",inset:0,zIndex:2,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"28px 20px",gap:14,textAlign:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"flex-end",gap:3,height:22}}>
            {[1,2,3,4,5].map((i)=>(
              <div key={i} style={{width:3,borderRadius:2,background:currentSlide.tagColor,animation:`eqBar${(i-1)%4+1} ${0.38+i*0.09}s ease-in-out infinite alternate`,boxShadow:`0 0 8px ${currentSlide.tagColor}88`}}/>
            ))}
          </div>
          <div style={{marginTop:4,padding:"4px 14px",background:currentSlide.tagColor+"18",border:`1px solid ${currentSlide.tagColor}30`,borderRadius:20,fontSize:10,fontWeight:700,color:currentSlide.tagColor,letterSpacing:1.5}}>
            {currentSlide.tag}
          </div>
        </div>
      </div>
  ...
  // ── §13 FIX #2 — LIVE PANEL (larger: more padding, bigger fonts, wider) ──
  const LivePanel = () => (
  ...
  const CarouselUI = ({ large }) => (
    <div style={{display:"flex",alignItems:"center",gap:20,background:"linear-gradient(135deg,#0e0e0e,#111)",border:"1px solid #1e1e1e",borderRadius:20,padding:large?"32px 28px":"28px 24px",position:"relative",overflow:"hidden",boxShadow:"0 4px 40px rgba(0,0,0,0.5)"}}>
      ...
      <div style={{flexShrink:0,width:large?340:300,height:large?340:300,position:"relative"}} onMouseEnter={()=>setPreviewHover(true)} onMouseLeave={()=>setPreviewHover(false)}>
        <img key={currentSingle.slug} src={currentSingle.cover} style={{width:"100%",height:"100%",borderRadius:large?18:16,objectFit:"cover",display:"block",boxShadow:large?"0 10px 50px rgba(0,0,0,0.7)":"0 8px 40px rgba(0,0,0,0.6)",transition:"filter 0.3s ease",filter:previewHover?"brightness(0.55)":"brightness(1)",animation:"fadeInCover 0.4s ease forwards"}}/>
        {/* §13 FIX #4 — uses openSingleModal to ensure single audio instance */}
        <div onClick={()=>openSingleModal(currentSingle)} style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,borderRadius:large?18:16,cursor:"pointer",opacity:previewHover?1:0,transition:"opacity 0.25s ease"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:large?14:12}}>
        <div key={`title-${currentSingle.slug}`} style={{fontSize:large?30:26,fontWeight:900,letterSpacing:2,animation:"fadeInUp 0.35s ease forwards"}}>{currentSingle.title}</div>
        ...
        <div style={{display:"flex",gap:6}}>
          {singles.map((s,i)=>(
            <div key={s.slug} onClick={()=>goToSingle(i,i>singleIndex?"right":"left")} style={{width:i===singleIndex?(large?24:20):(large?7:6),height:large?7:6,borderRadius:4,background:i===singleIndex?"#00ffff":"#333",cursor:"pointer",transition:"all 0.3s ease",boxShadow:i===singleIndex?"0 0 8px rgba(0,255,255,0.6)":"none"}}/>
          ))}
        </div>
        <div style={{display:"flex",gap:10,marginTop:large?8:6,flexWrap:"wrap"}}>
          <button onClick={()=>addToCart(currentSingle)} onMouseEnter={buttonHoverIn} onMouseLeave={buttonHoverOut} style={{padding:large?"11px 20px":"10px 18px",background:"#0a0a0a",color:"#00ffff",border:"1px solid #00ffff",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:"bold",transition:"0.25s"}}>+ Add to Cart</button>
          {large&&<button onClick={()=>addVinylToCart(currentSingle)} onMouseEnter={buttonHoverIn} onMouseLeave={buttonHoverOut} style={{padding:"11px 20px",background:"#0a0a0a",color:"#aaa",border:"1px solid #2a2a2a",borderRadius:8,cursor:"pointer",fontSize:13,transition:"0.25s"}}>+ Vinyl $47.99</button>}
        </div>
      </div>
  ...
  // ── §13 FIX #3 — RADIO CAROUSEL: ALL animations removed, static only ─────
  // narrow prop shrinks cover art width for the two-column FLOW STATE layout
  const RadioCarousel = ({ narrow=false }) => {
    const coverW    = narrow ? 200 : 320;
    const infoPad   = narrow ? "28px 22px" : "36px 32px";
    const titleSize = narrow ? 24 : 34;
    return (
      <div style={{position:"relative",borderRadius:22,overflow:"hidden",background:"linear-gradient(135deg,#080808,#0d0d0d)",border:"1px solid #1e1e1e",boxShadow:"0 8px 60px rgba(0,0,0,0.6)",height:"100%"}}>
        ...
        <div style={{display:"flex",alignItems:"stretch",minHeight:320,position:"relative",zIndex:1}}>
          {/* Cover art — NO animation keyframe (§13 FIX #3) */}
          <div style={{flexShrink:0,width:coverW,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{position:"absolute",top:16,left:16,background:currentSlide.tagColor,color:"#000",fontSize:9,fontWeight:900,letterSpacing:2.5,padding:"5px 12px",borderRadius:20,boxShadow:`0 0 16px ${currentSlide.tagColor}88`}}>{currentSlide.tag}</div>
          </div>
          {/* Info */}
          <div style={{flex:1,padding:infoPad,display:"flex",flexDirection:"column",justifyContent:"center",gap:14}}>
            <div style={{fontSize:10,color:"#444",letterSpacing:4,textTransform:"uppercase",fontWeight:700}}>2MRRW RADIO</div>
            {/* Title — NO animation (§13 FIX #3) */}
            <div style={{fontSize:titleSize,fontWeight:900,letterSpacing:2,lineHeight:1.1}}>{currentSlide.title}</div>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              {/* Equalizer — static bars, NO animation (§13 FIX #3) */}
              <div style={{display:"flex",alignItems:"flex-end",gap:3,height:18}}>
                {[10,16,8,14].map((h,i)=>(
                  <div key={i} style={{width:3,height:h,borderRadius:2,background:currentSlide.tagColor,boxShadow:`0 0 6px ${currentSlide.tagColor}88`}}/>
                ))}
              </div>
            ...
            <div style={{display:"flex",gap:10,marginTop:4}}>
              {/* §14 FLOW STATE trigger: hover on this button activates Conversion Mode */}
              <button
                onClick={()=>addToCart({title:currentSlide.title,slug:currentSlide.slug,cover:currentSlide.cover,price:currentSlide.price})}
            ...
            <div style={{display:"flex",gap:7,marginTop:10}}>
              {radioSlides.map((s,i)=>(
                <div key={s.slug} onClick={()=>goRadio(i)} style={{width:i===radioIndex?22:6,height:6,borderRadius:4,background:i===radioIndex?currentSlide.tagColor:"#2a2a2a",cursor:"pointer",transition:"all 0.3s ease",boxShadow:i===radioIndex?`0 0 8px ${currentSlide.tagColor}88`:"none"}}/>
              ))}
            </div>
          </div>
          {/* Arrows */}
  ...
  return (
    <>
      ...
      {/* §14 Cursor — trailing dot (slower CSS transition = trail effect) */}
      <div ref={cursorTrailRef} style={{position:"fixed",width:16,height:16,borderRadius:"50%",background:"radial-gradient(circle,rgba(0,255,255,0.10) 0%,transparent 70%)",pointerEvents:"none",transform:"translate(-50%,-50%)",zIndex:99998,mixBlendMode:"screen",transition:"left 0.18s ease,top 0.18s ease"}}/>
      ...
      {/* Hidden audio for now playing bar */}
      ...
      {/* SINGLE MODAL — §13 FIX #4: autoPlay REMOVED, modal audio is user-controlled only */}
      {selectedSingle && (
        <div onClick={()=>setSelectedSingle(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:8888,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div onClick={(e)=>e.stopPropagation()} style={{background:"#111",border:"1px solid #222",borderRadius:20,padding:30,width:340,display:"flex",flexDirection:"column",alignItems:"center",gap:14}}>
            ...
            {/* autoPlay intentionally absent — prevents double audio with nowPlayingAudioRef */}
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{flex:1,overflowY:"auto",padding:30}}>
            ...
            <div style={{position:"relative",height:380,marginBottom:0,borderRadius:20,overflow:"hidden",background:"black"}}>
              ...
              <div style={{position:"absolute",bottom:24,right:25,display:"flex",gap:16,alignItems:"center",zIndex:10,flexWrap:"wrap",justifyContent:"flex-end"}}>
                {SOCIALS.map((social)=>(
                  <a key={social.name} href={social.href} target="_blank" rel="noopener noreferrer" title={social.name} style={{color:"rgba(255,255,255,0.65)",transition:"transform 0.2s ease,color 0.2s ease,filter 0.2s ease",display:"flex",alignItems:"center",textDecoration:"none"}} onMouseEnter={(e)=>{e.currentTarget.style.transform="scale(1.5)";e.currentTarget.style.color="#00ffff";e.currentTarget.style.filter="drop-shadow(0 0 6px rgba(0,255,255,0.8))";}} onMouseLeave={(e)=>{e.currentTarget.style.transform="scale(1)";e.currentTarget.style.color="rgba(255,255,255,0.65)";e.currentTarget.style.filter="none";}}>{social.svg}</a>
                ))}
              </div>
            </div>
                    ...
                    <div style={{display:"flex",gap:18,alignItems:"flex-start"}}>
                      <div className="singles-row" style={{flex:1,display:"flex",gap:18,overflowX:"auto",paddingBottom:14,scrollSnapType:"x mandatory"}}>
                        {singles.map((single,i)=>(
                          // §13 FIX #1 — Container: border tint only. overflow:hidden REMOVED so img glow isn't clipped.
                          <div key={single.slug}
                            onClick={()=>openSingleModal(single)}
                            style={{flexShrink:0,width:200,cursor:"pointer",scrollSnapAlign:"start",opacity:0,animation:`fadeInUp 0.5s ease ${i*0.09}s forwards`,background:"#0a0a0a",borderRadius:14,border:"1px solid #1a1a1a",transition:"border-color 0.25s"}}
                            onMouseEnter={(e)=>{e.currentTarget.style.borderColor="#00ffff1a";}}
                            onMouseLeave={(e)=>{e.currentTarget.style.borderColor="#1a1a1a";}}>
                            {/* §13 FIX #1 — scale + glow ONLY on this <img>, never the parent */}
                            ...
                            />
                            <div style={{padding:"12px 14px 16px",borderRadius:"0 0 13px 13px"}}>
                      ...
                      {/* §13 FIX #2 — Live Panel: larger */}
                      <LivePanel/>
                    </div>
                  </div>

                  {/* §14 2MRRW RADIO — two-column with FLOW STATE */}
                  ...
                  <div id="home-exclusives">
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:18}}>
                      {exclusiveItems.map((item)=>(
                        <div key={item.id} style={{background:"#0d0d0d",border:"1px solid #1e1e1e",borderRadius:18,overflow:"hidden",cursor:"pointer",transition:"all 0.3s ease"}} onMouseEnter={(e)=>{e.currentTarget.style.borderColor=item.badgeColor+"55";e.currentTarget.style.boxShadow=`0 0 24px ${item.badgeColor}14`;e.currentTarget.style.transform="translateY(-2px)";}} onMouseLeave={(e)=>{e.currentTarget.style.borderColor="#1e1e1e";e.currentTarget.style.boxShadow="none";e.currentTarget.style.transform="none";}} onClick={()=>setExclusiveModal(item)}>
                          <div style={{position:"relative"}}>
              ...
              {activeTab==="exclusive"&&(
                <>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:20,marginTop:28}}>
                    {exclusiveItems.map((item)=>(
                      <div key={item.id} style={{background:"#0d0d0d",border:"1px solid #1e1e1e",borderRadius:20,overflow:"hidden",cursor:"pointer",transition:"all 0.3s ease"}} onMouseEnter={(e)=>{e.currentTarget.style.borderColor=item.badgeColor+"55";e.currentTarget.style.boxShadow=`0 0 28px ${item.badgeColor}18`;e.currentTarget.style.transform="translateY(-3px)";}} onMouseLeave={(e)=>{e.currentTarget.style.borderColor="#1e1e1e";e.currentTarget.style.boxShadow="none";e.currentTarget.style.transform="none";}} onClick={()=>setExclusiveModal(item)}>
                        <div style={{position:"relative"}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:20}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                        {[{label:"My Music Library",tab:"mymusic",color:"#00ffff"},{label:"Exclusive Drops",tab:"exclusive",color:"#a259ff"},{label:"The Circle",tab:"circle",color:"#ff6b35"},{label:"Inner Circle",tab:"innercircle",color:"#a259ff"}].map((link)=>(
                          <button key={link.tab} onClick={()=>switchTab(link.tab)} style={{padding:"16px 18px",background:"#0a0a0a",border:`1px solid ${link.color}22`,borderRadius:14,cursor:"pointer",textAlign:"left",color:link.color,fontSize:13,fontWeight:700,transition:"0.2s"}} onMouseEnter={(e)=>{e.currentTarget.style.borderColor=link.color+"55";e.currentTarget.style.background=link.color+"0a";}} onMouseLeave={(e)=>{e.currentTarget.style.borderColor=link.color+"22";e.currentTarget.style.background="#0a0a0a";}}>
                            {link.label} →
                          </button>
                        ))}
                      </div>
          ...
          {nowPlaying&&(
            <div style={{flexShrink:0,borderTop:"1px solid #141414",background:"rgba(4,4,4,0.95)",backdropFilter:"blur(20px)",padding:"10px 20px",display:"flex",alignItems:"center",gap:14,boxShadow:"0 -4px 30px rgba(0,0,0,0.5)"}}>
              ...
              <button onClick={()=>{if(!nowPlayingAudioRef.current)return;if(nowPlayingPlaying){nowPlayingAudioRef.current.pause();setNowPlayingPlaying(false);}else{nowPlayingAudioRef.current.src=nowPlaying.preview;nowPlayingAudioRef.current.play().catch(()=>{});setNowPlayingPlaying(true);}}} style={{width:36,height:36,borderRadius:"50%",background:"#00ffff",border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
                {nowPlayingPlaying
        ...
        <div style={{width:240,flexShrink:0,borderLeft:"1px solid #222",padding:25,overflowY:"auto",background:"rgba(4,4,4,0.8)",backdropFilter:"blur(12px)"}}>
          ...
          {cart.map((item,i)=>(
            <div key={i} style={{marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
              ...
              <button onClick={()=>removeFromCart(i)} onMouseEnter={(e)=>{e.currentTarget.style.color="#fff";e.currentTarget.style.textShadow="0 0 8px rgba(255,255,255,0.8)";}} onMouseLeave={(e)=>{e.currentTarget.style.color="#666";e.currentTarget.style.textShadow="none";}} style={{background:"none",border:"none",color:"#666",fontSize:16,cursor:"pointer",marginLeft:"auto",transition:"0.2s"}}>×</button>
            </div>
          ))}
      ...
      <style jsx>{`
        ...
        /* Equalizer — FLOW STATE now-playing only */
...
// ── GRID ──────────────────────────────────────────────────────────────────────
// §13 FIX #1: hoverIn/hoverOut passed into <img> directly.
// Container div gets ONLY a subtle border shift — no glow, no scale on parent.
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:22}}>
      {items.map((item)=>(
        <div key={item.slug}
          ...
          onMouseLeave={(e)=>{e.currentTarget.style.borderColor="#1a1a1a";}}>
          {/* §13 FIX #1 — scale + glow on <img> only */}
          <img src={item.cover}
