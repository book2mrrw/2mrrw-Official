-- 2MRRW: Auth-linked commerce, library entitlements, secure access
...
-- Legacy app used one row per cart item (item_slug, item_title, …). Rename so the new schema can be created.
do $$
begin
  if exists (
...
-- ── ACCESS TOKENS (QR / secure deep links) ─────────────────────────────────────
...
-- Access tokens: users see own tokens
...
-- Service role bypasses RLS for webhooks/admin routes

-- ── STORAGE BUCKET (run separately if needed) ─────────────────────────────────
-- insert into storage.buckets (id, name, public) values ('digital-assets', 'digital-assets', false);
-- RLS on storage.objects: authenticated users with library entitlement only via signed URLs from API
