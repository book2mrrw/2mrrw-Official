-- F0: forward-only reconciliation for migration 41 drift.
-- Restores the atomic, fenced worker RPCs without rewriting migration history.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.account_lifecycle_steps'::regclass
      and conname = 'account_lifecycle_steps_max_attempts_check'
  ) then
    alter table public.account_lifecycle_steps
      add constraint account_lifecycle_steps_max_attempts_check
      check (max_attempts between 1 and 32) not valid;
  end if;
end $$;

alter table public.account_lifecycle_steps
  validate constraint account_lifecycle_steps_max_attempts_check;
...
alter table public.account_lifecycle_requests
  validate constraint account_lifecycle_requests_user_id_fkey;
...
comment on function public.finish_account_lifecycle_step(uuid,text,uuid,jsonb,boolean) is
  'Completes a lifecycle step only when its unexpired fencing token matches.';
comment on function public.retry_account_lifecycle_step(uuid,text,uuid,text) is
  'Schedules bounded exponential retry only when its unexpired fencing token matches.';
