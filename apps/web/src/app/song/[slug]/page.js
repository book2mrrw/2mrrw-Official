...
export async function generateMetadata({ params }) {
  ...
  if (!(await requireConsumerPrincipal())) redirect(loginRedirectPath(`/song/${slug}`));
  try {
    ...
    const image = data.cover_url
      ...
      : "/icons/icon-512.png";
    const canonical = `${BASE_URL}/song/${slug}`;
...
export default async function SongDeepLinkPage({ params }) {
  const { slug } = await params;
  if (!(await requireConsumerPrincipal())) redirect(loginRedirectPath(`/song/${slug}`));
