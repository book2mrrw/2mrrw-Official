...
function ReplaceMasterModal({ release, onClose, typeLabels }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: C.muted, marginBottom: 20 }}>
          ...
          {" · "}
          {typeLabels[release?.release_type] || release?.release_type}
        </div>
...
// ── Replace Cover Modal ────────────────────────────────────────────────────────
function ReplaceCoverModal({ release, onClose, typeLabels }) {
...
// ── Main Component ──────────────────────────────────────────────────────────────
export default function ReleasesManager({ kind = "music" }) {
  const isPodcast = kind === "podcast";
  const typeLabels = TYPE_LABELS_BY_KIND[kind] || TYPE_LABELS_BY_KIND.music;
  const uploadHref = isPodcast ? "/admin/upload?kind=podcast" : "/admin/upload";
  const draftHref  = (id) => isPodcast ? `/admin/upload?draft=${id}&kind=podcast` : `/admin/upload?draft=${id}`;
  const backHref   = isPodcast ? "/admin" : "/admin";
  ...
  const loadReleases = async () => {
    setLoading(true);
    try {
      const res = await fetch(isPodcast ? "/api/admin/releases?kind=podcast" : "/api/admin/releases");
    ...
    <div style={{ minHeight: "100vh", background: C.bg, padding: "40px 20px", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
          <div>
            <a href={backHref} style={{ fontSize: 11, color: C.muted2, textDecoration: "none", letterSpacing: "0.08em", fontWeight: 700, textTransform: "uppercase" }}>← Admin</a>
            <h1 style={{ fontSize: 24, fontWeight: 800, color: C.text, margin: "6px 0 0" }}>{isPodcast ? "Manage Podcast" : "Manage Releases"}</h1>
          </div>
          <button
          ...
          >
            {isPodcast ? "+ Upload Episode" : "+ Upload Release"}
          </button>
        </div>
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: C.muted }}>Loading {isPodcast ? "episodes" : "releases"}…</div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: C.muted }}>No {isPodcast ? "podcast episodes" : "releases"} found.</div>
        ) : (
            ...
            {filtered.map((rel, i) => {
              ...
              const rowEl = (
                <div
                  ...
                  onClick={isDraft ? () => router.push(draftHref(rel.id)) : undefined}
                  onKeyDown={isDraft ? (e) => {
                    if (e.key === "Enter" || e.key === " ") { e.preventDefault(); router.push(draftHref(rel.id)); }
                  } : undefined}
                  style={{
                    display: "grid",
                    gridTemplateColumns: "1fr 90px 100px 110px 180px",
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: C.muted }}>{typeLabels[rel.release_type] || rel.release_type}</div>
                  ...
                  >
                    ...
                    {rel.status === "draft" && (
                      <button
                        onClick={() => router.push(draftHref(rel.id))}
                        style={{
                          background: C.surface2, border: `1px solid ${C.border2}`, borderRadius: 6,
      ...
      {undoDump ? (
        <div role="status" style={{ position: "fixed", left: "50%", bottom: 26, transform: "translateX(-50%)", zIndex: 1000, display: "flex", alignItems: "center", gap: 18, background: "rgba(12,12,12,.96)", border: `1px solid ${C.accentBorder}`, boxShadow: "0 16px 50px rgba(0,0,0,.55)", borderRadius: 12, padding: "13px 16px", color: C.text, fontSize: 13 }}>
          <span>{isPodcast ? "Episode dumped" : "Draft dumped"}</span>
