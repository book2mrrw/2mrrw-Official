...
4. Replace the entire policy with the contents of `r2-cors-policy.json` (or `docs/reports/r2-cors-policy-recommended.json` in the repo).
5. Click **Save**.
6. Wait ~1 minute for propagation, then run `./verification-curls.sh` or inspect `curl-probes.txt`.

## Verify in dashboard

After save, confirm the policy includes:
...
Set on Vercel for the storefront:
